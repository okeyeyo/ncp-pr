...
public class MyController {
	...
	// 업데이트 기능을 위한 요청 진행하기
	// 1. 요청 Mapping
	// 2. 응답의 구조 생성 (form 태그를 통해 입력받은 데이터를 활용)
	// 3. DB에 해당하는 데이터 전달 -> update sql구분 실행
...
//		mem.setEmail(mem2.getEmail()); // 기존의 이메일이 아닌 받아온 이메일로 수정할 수 있게끔
		
		MsgMember member = mapper.update(mem);
		logger.info("수정 된 회원의 정보 >> " + member.toString());
	...
	public String delete(@RequestParam("email") String email) {
		...
		// 쿼리 파라미터 중 emile 값을 String name 변수 바인딩
		mapper.delete(email);
